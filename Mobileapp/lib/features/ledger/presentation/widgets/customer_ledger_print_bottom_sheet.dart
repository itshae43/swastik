...
class CustomerLedgerPrintBottomSheet extends StatefulWidget {
  final PartyModel party;
  final List<TransactionModel> initialTransactions;

  const CustomerLedgerPrintBottomSheet({
    super.key,
    required this.party,
  ...
  @override
  State<CustomerLedgerPrintBottomSheet> createState() => _CustomerLedgerPrintBottomSheetState();
}

class _CustomerLedgerPrintBottomSheetState extends State<CustomerLedgerPrintBottomSheet> {
  ...
  List<TransactionModel> get filteredTxns {
    final partyTxns = widget.initialTransactions.where((t) => t.partyId == widget.party.id).toList();

    var filtered = partyTxns.where((t) {
          ...
          children: [
            Row(
              ...
              children: [
                Expanded(
                  child: Text(
                    'Export Ledger: ${widget.party.name}',
                      ...
                      onPressed: () async {
                        ...
                        try {
                          ...
                          final success = await PdfService.savePdfToDownloads(
                            pdfBytes,
                            'Ledger_${widget.party.name.replaceAll(' ', '_')}_${DateTime.now().millisecondsSinceEpoch}',
                          );
                          setState(() {
                            isPrinting = false;
                            isSaved = success;
...
void showCustomerLedgerPrintBottomSheet(BuildContext context, PartyModel party, List<TransactionModel> initialTransactions) {
  showModalBottomSheet(
    context: context,
    isScrollControlled: true,
    backgroundColor: Colors.transparent,
    builder: (context) {
      return CustomerLedgerPrintBottomSheet(
        party: party,
